...
function App() {
  ...
  const handleCopyTranscript = () => {
    if (!result) return;
    const fullText = result.segments.map(s => `[${s.timestamp}] ${s.speaker}: ${s.content}`).join('\n\n');
        ...
        <div className="space-y-8">
          ...
          {result && status === 'success' && (
            <div>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <div>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                        {result.segments.reduce((acc, seg) => acc + seg.content.length, 0).toLocaleString()} characters
                      </p>
                    </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-950 p-2 -m-2 rounded-xl">
